# Ford Motor Credit Company Software Engineering Internship Exercise (Manhattan Distance)
*Written By Justin Kula*

Installation and Operating Instructions:
 - Install Python3, if it is not already installed on your local machine (https://www.python.org/downloads/)
 - Clone this git repo by executing: `git clone https://github.com/kulaj/ford-2021-exercise.git`
 - Navigate to the project directory in terminal by executing: `cd ford-2021-exercise`
 - Depending on your python3 installation and operating system, the command to run the application will be one of the following: 
   - `python3 manhattan.py` (typically required for macOS or Linux installations)
   - `py -3 manhattan.py` (typically required for Windows installations)

Unit Tests:
- To run the test suite, execute either `python3 tests.py` or `py -3 tests.py`, depending on your operating system.

Example Screenshot (includes input error demonstration):
![image](https://user-images.githubusercontent.com/25033887/111053006-3788b400-842e-11eb-8ce2-7b726cb12ebe.png)
